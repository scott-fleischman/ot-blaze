...
template<typename T, size_t S>
void TestSignedBigEndian(T expected, std::array<uint8_t, S> bytes)
{
	auto big_endian = ot::data_type::signed_big_endian<T, S> { bytes };
	EXPECT_EQ(expected, big_endian.GetValue());
}
...
TEST(SignedBigEndian, 4Byte_int_fast32_t)
{
	TestSignedBigEndian<int_fast32_t, 4>(0x04030201, { 0x04, 0x03, 0x02, 0x01 });
}

TEST(SignedBigEndian, 4Byte_Negative_int_fast32_t)
{
	TestSignedBigEndian<int_fast32_t, 4>(-2, { 0xff, 0xff, 0xff, 0xfe });
}
